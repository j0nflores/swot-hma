...
def extract_cama_swot(reach):
    ...
    if not os.path.exists(output_file):
        ...
        try:
            ...
            df['dam_era_wse'] = dam_era_data[:,reach_df.d_iy1,reach_df.d_ix1].reshape(-1).astype('float')
            df['dam_vic_wse'] = dam_vic_data[:,reach_df.d_iy1,reach_df.d_ix1].reshape(-1).astype('float')
            df['nat_wse'] = nat_data[:,reach_df.d_iy1,reach_df.d_ix1].reshape(-1).astype('float')
...
output_folder = f'./output/swot_cama/{year}'
